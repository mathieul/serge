defmodule Serge.Scrumming.StoryTest do
  ...
  describe "list_stories/1" do
    test "it returns all stories for a team ordered by sort", %{team: team} do
      insert(:story, team: team, description: "one", sort: 2.0)
      insert(:story, description: "two")
      insert(:story, team: team, description: "three", sort: 1.5)

      assert Enum.map(Scrumming.list_stories(team: team), &(&1.description)) == ["three", "one"]
    end
  end
end
